...
{
    // ─── Configuración (leída desde config/traffic-reader.php) ──────────────────────────
    ...
    private function record(Request $request, int $statusCode): void
    {
        try {
            ...
            // Ignorar rutas configuradas por el usuario del paquete
            ...
            $threats = array_merge(
                ...
                $this->detectRateAnomalies($request, $statusCode),
            );
    ...
    // ─── Detección de anomalías con estado (Cache) ────────────────────────────

    private function detectRateAnomalies(Request $request, int $statusCode): array
    {
        $threats = [];
        $ip      = $request->ip();
        $now     = now();
        ...
        $rateKey   = "traffic-reader:rate:{$ip}:{$window}";
        ...
        // Muchos 404 en la misma hora
        if ($statusCode === 404) {
            $nfKey   = "traffic-reader:404:{$ip}:{$hour}";
        ...
        // Acceso repetido a rutas sensibles
        $path = $request->path();
        foreach ($this->sensitiveRoutes as $route) {
            if (stripos($path, $route) !== false) {
                $probeKey   = "traffic-reader:probe:{$ip}:{$hour}";
        ...
        // Brute-force: muchos 401/403
        if (in_array($statusCode, [401, 403], true)) {
            $bfKey   = "traffic-reader:bf:{$ip}:{$hour}";
